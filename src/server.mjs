...
function normalizeDeparture(input) {
  return {
    ...
    destination: required(input.destination, 'Destino'),
    origin_name: required(input.originName, 'Origen'),
    origin_code: required(input.originCode, 'CÃ³digo de origen').toUpperCase(),
    ...(input.accommodationName ? { accommodation_name: input.accommodationName } : {}),
    ...(input.itinerarySummary ? { itinerary_summary: input.itinerarySummary } : {}),
    ...(input.activitiesSummary ? { activities_summary: input.activitiesSummary } : {}),
    starts_at: required(input.startsAt, 'Fecha de inicio'),
    ends_at: required(input.endsAt, 'Fecha de fin'),
    price_per_traveller: Number(required(input.pricePerTraveller, 'PVP por viajero')),
    ...
    deposit_amount: Number(required(input.depositAmount, 'Deposito')),
